...
export const BondSupply = () => {
    ...
    () =>
      tokens.map(token => ({
        ...
        totalSupplied: bondBalances[token.address]
          ...
          : '0',
        apy: apyFromApr(sumAPRs(bondAPRs[token.address], incentiveAPRs[token.address]) ?? 0, 365),
  ...
  const averageYield = useMemo(() => {
    ...
    const avgYield = tokens.reduce((acc, cur) => {
      const apy = apyFromApr(sumAPRs(bondAPRs[cur.address], incentiveAPRs[cur.address]), 365)
